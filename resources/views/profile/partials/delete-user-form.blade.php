<section>
    <header>
        ...
        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('Search and delete users from the system.') }}
        </p>
    </header>
    ...
    <div class="mt-6 space-y-3" id="usersList">
        @if($users && $users->count() > 0)
            @foreach($users as $userItem)
                <div class="user-item flex items-center justify-between p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow" data-username="{{ strtolower($userItem->name) }}">
                    <div class="flex items-center space-x-4 flex-1">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">
                                {{ $userItem->name }}
                                @if($userItem->admin)
                                    <span class="ml-2 px-2 py-1 text-xs font-semibold rounded-full bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                                        Admin
                                    </span>
                                @endif
    ...
    <div id="deleteModal" class="hidden fixed inset-0 bg-gray-500 dark:bg-gray-900 bg-opacity-75 dark:bg-opacity-75 overflow-y-auto h-full w-full z-50 px-4">
                ...
                <div class="mt-3">
                    ...
                    <div class="mt-2 px-7 py-3">
                        <p class="text-sm text-gray-600 dark:text-gray-400 text-center">
                            You are about to delete user <span id="deleteUsername" class="font-semibold text-gray-900 dark:text-gray-100"></span>.
                        </p>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2 text-center">
                            Once the account is deleted, all of its resources and data will be permanently deleted. This action cannot be undone!
                        </p>
                    </div>
    ...
    @if(session('status') == 'user-deleted')
        <div class="mt-4 p-4 bg-green-50 dark:bg-green-900/50 border border-green-200 dark:border-green-800 text-green-800 dark:text-green-200 rounded-lg">
            User successfully deleted
        </div>
    @endif

    @if(session('error'))
        <div class="mt-4 p-4 bg-red-50 dark:bg-red-900/50 border border-red-200 dark:border-red-800 text-red-800 dark:text-red-200 rounded-lg">
            {{ session('error') }}
        </div>
    @endif
</section>
